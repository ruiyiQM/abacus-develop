#!/usr/bin/env python3
"""Fetch and verify resources declared by a pinned FDE example manifest."""
...
def install_resources(
            ...
            if source_root is not None:
                ...
                if not source.is_file():
                    raise FileNotFoundError(
                        f"resource is missing from source root: {source}")
                ...
                request = Request(
                    url, headers={"User-Agent": "ABACUS-FDE-example/1"})
    ...
    installed_manifest["installed_files"] = [
        str(path.resolve()) for path in installed]
...
def main(
    ...
    parser.add_argument(
        ...
        required=default_destination is None)
    parser.add_argument(
    ...
    if arguments.manifest is None:
        parser.error("a resource manifest is required")
    ...
    try:
        manifest = load_manifest(arguments.manifest.resolve(strict=True))
        installed = list(install_resources(
            manifest, arguments.destination.resolve(), source_root,
            arguments.force))
