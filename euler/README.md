...
SCF threshold.  It then makes a strict one-step CPU bootstrap for PW91k and
revAPBEk, rejects the checkpoint unless its spin-density range exceeds
`1e-8` bohr^-3, exercises the CPU PW FFT and single-rank cuFFT paths from that
same nonuniform density, and requires their reported energies to agree within
