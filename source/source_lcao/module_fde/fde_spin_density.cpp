...
{
    if (local_size != static_cast<std::size_t>(basis.nrxx)
        || basis.nxyz <= 0
        || (local_size != 0 && (alpha_density == nullptr || beta_density == nullptr))
        || alpha_electrons < 0 || beta_electrons < 0
        || !std::isfinite(cell_volume_bohr3) || cell_volume_bohr3 <= 0.0)
    {
    ...
    prepare_density(alpha_density, local_size, sums[0], invalid);
    prepare_density(beta_density, local_size, sums[1], invalid);
    ...
    const double volume_element
        = cell_volume_bohr3 / static_cast<double>(basis.nxyz);
